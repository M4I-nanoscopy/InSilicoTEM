...
% %  ----------size N/cos(tiltang) by N -------------------------
volstruct = zeros(floor((N+mindista)/cos(max(params2.acquis.tilt))), N+mindista, szz+1);
volstructmmb = zeros(floor((N+mindista)/cos(max(params2.acquis.tilt))),N+mindista, szz+1);

% %  ----------size N by N -------------------------
% volstruct = zeros(N, N, szz+1);
% volstructmmb = zeros(N ,N , szz+1);
...
%     save([pwd filesep 'Particles' filesep TransName], 'transl'); 
    ...
    for ss=1:numPart
        ...
        xy0 = [floor((N+mindista)/cos(max(params2.acquis.tilt))/2)+xt(ss) floor((N+mindista)/2)+yt(ss)]+1;  % for size N/cos(tiltang) by N
%        xy0 = [floor(N/2)+xt(ss) floor(N/2)+yt(ss)]+1; % for size N by N
        xy1 = xy0 + [nx ny] - [1,1];
...
%                 floor((size(volstruct,1)-xlong)/2):floor((size(volstruct,1)+xlong)/2), ]);
            volstruct = volstruct(1:floor((N+mindista)/cos(max(params2.acquis.tilt))), 1:floor((N+mindista)/cos(max(params2.acquis.tilt))), :);
    ...
    fclose(fPartSpec); 
    volstruct = cut(volstruct, [N+mindista floor((N+mindista)/cos(max(params2.acquis.tilt))) szz+1]);  % for size N/cos(tiltang) by N
%    volstruct = cut(volstruct, [N N szz+1]);  % for size N by N
