function [VolPot, PosOrient] = generateFullVolume_DL(PartPot,params2)
...
if strcmp(params2.spec.source, 'pdb')
    ...
    for ss=1:numPart
        ...
        %         rotate the partice 90 degrees
        if params2.particle.rotate ~= 0
            atompot4p = zeros(nx,nz,ny);
            for ii = 1:nx
                simg = squeeze(atompot4(ii,:,:));
                atompot4p(ii,:,:) = imrotate(simg,params2.particle.rotate);                
            end
            temp = ny;
            ny = nz;
            nz = temp;
        ...
        %         find index of atompot in the full volume
    ...
    if params2.spec.imagpot == 3
        volstruct(volstruct==0) =  1i*min(atompotI(:)); %params2.spec.potenampl; %DL
    end
